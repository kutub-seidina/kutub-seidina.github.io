...
function syncSubmissionMetadata() {
  ...
  if (submitLang) {
    submitLang.value = lang;
  }

  if (submitSubject) {
    submitSubject.value = KutubI18n.t("formsubmit.subject");
  }
}
...
function resetImagePreview({ clearInput = false, clearValidation = true } = {}) {
  if (previewObjectUrl) {
  ...
  if (clearInput) {
    bookCover.value = "";
    updateDraftImageState({ hadImage: false, showWarning: false });
  }
...
async function validateSelectedImage() {
  resetImagePreview({ clearInput: false, clearValidation: true });
  ...
  updateDraftImageState({ hadImage: true, showWarning: false });
...
function updateReplyContactField({ clearValidation = true } = {}) {
  if (clearValidation) {
...
function updateDraftImageState({ hadImage, showWarning = false }) {
...
removeImage.addEventListener("click", () => {
  resetImagePreview({ clearInput: true, clearValidation: true });
  if (errors.bookIdentity && !errors.bookIdentity.hidden) {
    validateBookIdentity();
  }
  checkAndHideSummaryError();
...
bookReportForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    if (firstInvalid) {
      ...
      firstInvalid.focus();
      firstInvalid.scrollIntoView({ behavior: getScrollBehavior(), block: "center" });
      return;
    }
...
if (reportFormLink && reportFormTitle) {
  reportFormLink.addEventListener("click", event => {
    ...
    event.preventDefault();
    reportFormTitle.scrollIntoView({ behavior, block: "start" });
    setTimeout(() => {
      reportFormTitle.focus({ preventScroll: true });
    }, behavior === "auto" ? 0 : 300);
  });
}
...
KutubI18n.subscribe(lang => {
  ...
  updateReplyContactField({ clearValidation: false });
